...
- **new** 侧边栏连接项和起始页卡片新增悬停"..."按钮，点击弹出右键菜单。
- **new** 文件列表支持 Windows 隐藏文件检测，本地和远程 Windows SFTP 服务端均可识别。
- **improve** 统一修改分组对话框：起始页卡片复用侧边栏的完整分组对话框，支持新建分组。
- **improve** 去掉 SFTP 右键菜单中与工具栏重复的刷新和上传项。
- **improve** SFTP 传输进度条样式优化：固定行高防抖，统一图标与字号风格。
- **improve** 右侧菜单"复制会话"英文缩写为 Duplicate。
- **improve** 所有非 lucide 图标（原始 SVG、emoji、文本符号）统一替换为 lucide 图标组件。
- **bugfix** 修复起始页卡片编辑保存不生效的问题。
